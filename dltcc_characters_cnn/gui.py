...
def predict(canvas, canvas_compare):
    ...
    input_array = np.reshape(input_array, [batch_size, input_bitmap.shape[0] * input_bitmap.shape[1]])

    if input_array is None:
        print("Input should not none!")

    # prediction shape: [batch_size, width * height]
    prediction = sess.run(models.out, feed_dict={x: input_array})
    ...
    pred_arr = np.reshape(pred_arr, prediction.shape[1] * prediction.shape[2])

    # minPredVal = np.amin(pred_arr)
    # maxPredVal = np.amax(pred_arr)
    # prediction_normed = _normalize_func(pred_arr, minVal=minPredVal, maxVal=maxPredVal)
    # shape (8000,)
